...
later module genuinely needs more. **This cap is a safety habit, not just a cost control:** every
authored exercise must include a human review/approval checkpoint before the sweep actually fans
out to its full run count, not just a printed estimate the learner can ignore - a scripted,
non-interactive loop should never run fully unattended up to the cap without a deliberate go-ahead.

All haystack/filler content used in any exercise - including tool-output filler used to reach a real
compaction trigger in Module 04, not just the needle-in-a-haystack text in Modules 02-03 - must be
synthetic or public-domain (e.g. essay corpora, generated lorem-style filler, or synthetic file
trees), never real logs, internal docs, real tool output against real repos, or credential-shaped
strings, even for realism or convenience.
...
| 02 | [Positional Bias - Lost in the Middle](02-positional-bias/README.md) | 01 | Does retrieval accuracy actually drop when the fact I need is buried mid-context, on the harness I actually use? | A sweep of 5 depths × 2 lengths (10 cells), 2 trials per cell (20 runs, the full ≤20 cap), scores retrieval accuracy per depth (empirical) + Coachgremlin confirms a pre-registered prediction was recorded before running, the experiment held every variable but depth constant, and the learner correctly explains *whichever* result they got - a flat curve is a valid, real outcome on frontier models, not a failed exercise (conceptual) |
| 03 | [Length-Driven Context Rot](03-length-driven-context-rot/README.md) | 02 (isolating length requires already controlling for position) | Does accuracy degrade with length alone, and does a coherent haystack actually help or hurt? | A capped sweep varies length across ≥3 points spanning a 4x range, 2 trials per length, at a depth chosen by a stated rule (Module 02's worst-scoring depth, or the 50% default if flat), plus one coherent-vs-shuffled comparison, both scored (empirical) + Coachgremlin confirms the depth-selection rule was followed and the learner reports the shuffled-vs-coherent result honestly, whichever direction it comes out, rather than assuming it must match the Chroma report (conceptual) |
| 04 | [Compaction Mechanics](04-compaction-mechanics/README.md) | 01 (needs window vocabulary); benefits from 02+03's intuition, not hard-blocked by them | When my harness compacts for real, what actually survives and what's silently lost? | A real session (synthetic filler only) plants a fact before the compaction trigger, drives past it, and probes recall against **two** controls - pre-compaction, and a same-distance compaction-not-triggered control that isolates compaction-specific loss from ordinary long-context decay - each repeated ≥3 trials (empirical) + Coachgremlin confirms the learner can name *which* categories of content survived vs. were dropped, and that the conclusion is attributed to compaction specifically, not distance alone (conceptual) |
| 05 | [Mitigation Strategies](05-mitigation-strategies/README.md) | at least one of 02, 03, or 04 (a mitigation's "it helped" claim needs an already-measured baseline; doesn't require all three) | Does a real mitigation (retrieval, sub-agent isolation, structured notes) actually beat the naive baseline I already measured? | The learner applies one mitigation to a failure mode measured in 02, 03, or 04, reruns that module's own scoring script, and reports a before/after delta (empirical) + Coachgremlin confirms the comparison is apples-to-apples (same variable, same scoring method as the original baseline) and the conclusion (helped / didn't / inconclusive) is honestly stated (conceptual) |
...
| 02 | Positional Bias | A reusable needle-in-a-haystack test harness script, parameterized for the learner's own tasks |
| 03 | Length-Driven Context Rot | A "safe context budget" measurement script - the learner's own empirical answer to how much of a model's window can actually be trusted |
